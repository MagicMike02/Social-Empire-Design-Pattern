...
namespace Script.EconomySystem
{
	...
	public class GameEconomyManager : MonoBehaviour
	{
...
#if UNITY_EDITOR
			// Unico log con tutte le risorse
			var resourceList = string.Join(", ", _resources.Select(kvp => $"{kvp.Key}={kvp.Value}"));
			Debug.Log($"[GameEconomyManager] Initialized. Resources: [{resourceList}]");
#endif
		}
		...
		/// Genera eventi GlobalEventBus per ogni risorsa spesa.
		/// </summary>
